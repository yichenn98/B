...
const DEFAULT_SHOPS = ['50嵐', '一沐日', '五桐號', '迷客夏', '可不可', '得正', '麻古茶坊'];
...
const App: React.FC = () => {
  ...
  return (
    <div className="max-w-xl mx-auto px-6 pt-[calc(1.5rem+env(safe-area-inset-top))] pb-[calc(3rem+env(safe-area-inset-bottom))] space-y-12">
      ...
      <section className="bg-white rounded-[3.5rem] p-6 sm:p-10 card-shadow border border-stone-100">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button
                ...
                className={`relative h-14 sm:h-16 border border-transparent flex flex-col items-center justify-center transition-all ${
                  isSelected ? 'bg-stone-100 rounded-[1.2rem] border-stone-200 z-10 scale-105 shadow-sm' : 'active:bg-stone-50'
                }`}
              >
      ...
      {selectedDate && (
        <div className="space-y-6">
          <div className="bg-stone-700 p-8 sm:p-10 rounded-[3rem] card-shadow text-white">
          ...
          <div className="space-y-4">
            {getRecordsForDate(selectedDate).map((r, i) => (
              <div key={r.id} className="bg-white p-6 sm:p-8 rounded-[2.5rem] card-shadow border border-stone-100 flex justify-between items-center">
                <div className="space-y-1 flex-1 pr-4">
                  <p className="text-[10px] font-black text-stone-400 uppercase tracking-[0.25em]">Prescription {i + 1}</p>
                  <h5 className="font-black text-stone-700 text-lg sm:text-xl">{r.shop} {r.item}</h5>
                  <p className="text-xs sm:text-sm text-stone-500 font-bold">甜度：{r.sweetness}；冰塊：{r.ice}</p>
                </div>
                <div className="flex items-center space-x-4">
                   <span className="text-2xl sm:text-3xl font-black text-stone-700 tracking-tighter">${r.price}</span>
            ...
            {getRecordsForDate(selectedDate).length < 2 && (
              <button onClick={() => setIsFormOpen(true)} className="w-full py-8 border-2 border-dashed border-stone-200 rounded-[2.5rem] text-stone-400 flex items-center justify-center space-x-4 bg-white/40 active:bg-stone-50">
                <div className="p-2 rounded-full bg-stone-100"><Icons.Plus /></div>
                <span className="font-black text-xs tracking-[0.3em] uppercase">Add Prescription</span>
              </button>
            )}
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 gap-4 sm:gap-6">
        <button onClick={() => setAnalyticsType('shop')} className="bg-white p-6 sm:p-8 rounded-[2.5rem] card-shadow border border-stone-100 space-y-2 text-left active:scale-95">
          <p className="text-[10px] font-black text-stone-400 uppercase tracking-[0.3em]">Clinic Favorite</p>
          <p className="text-base sm:text-lg font-black text-stone-700 truncate">{favoriteShop.name}</p>
        </button>
        <button onClick={() => setAnalyticsType('item')} className="bg-white p-6 sm:p-8 rounded-[2.5rem] card-shadow border border-stone-100 space-y-2 text-left active:scale-95">
          <p className="text-[10px] font-black text-stone-400 uppercase tracking-[0.3em]">Addiction Item</p>
          <p className="text-base sm:text-lg font-black text-stone-700 truncate">{favoriteItem.name}</p>
        </button>
      </section>

      {analyticsType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 backdrop-blur-md p-6">
          <div className="w-full max-w-lg bg-white rounded-[3rem] p-8 card-shadow max-h-[70vh] flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-stone-700">{analyticsType === 'shop' ? '店家統計' : '飲品統計'}</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2">
              {allAnalyticsData.map((data, idx) => (
                <div key={data.name} className="flex items-center justify-between p-4 bg-stone-50 rounded-[1.5rem]">
                  <span className="font-bold text-stone-600 text-sm">#{idx + 1} {data.name}</span>
                  <span className="text-xs font-black text-stone-400">{data.count} 次</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-stone-900/40 backdrop-blur-md p-0 sm:p-6">
